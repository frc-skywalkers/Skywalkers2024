...
public class Lightstrip extends SubsystemBase {
  ...
  private void update(LedState state, Timer timer, Range range) {
    ...
    if(state.getEffect() == "Solid") {
      candle.setLEDs(state.getRed(), state.getGreen(), state.getBlue(), 0, range.getStart(), range.getEnd() - range.getStart() + 1);
      ...
      candle.setLEDs((int)(state.getRed() / ((timer.get() % 0.5) * 2)), (int)(state.getGreen() / ((timer.get() % 0.5) * 2)), (int)(state.getBlue() / ((timer.get() % 0.5) * 2)), 0, range.getStart(), range.getEnd() - range.getStart() + 1);
    } else if(state.getEffect() == "Signal") {
      int chain = 0;

      if((timer.get() % 1) < 0.125) { //i know there is prob a better way to do this, but im too lazy to
        chain = 0;
      ...
      for(int i = 0; i < range.getStart() - range.getEnd() + 1; i++) {
        ...
        candle.setLEDs((int)(state.getRed() * chain / 6), (int)(state.getGreen() * chain / 6), (int)(state.getBlue() * chain / 6), 0, range.getStart() + i, 1);
